...
Logic:
    ...
    We fill up the the interval array with smallest picks from given arrays, finding the difference, then replacing the smallest among the element with the second smallest from same array it belonged from. Repeat process until all arrays are exhausted.
...
def find_closest_elements_in_sorted_arrays_original(sorted_arrays: List[List[int]]
    ...
    for idx, sorted_array in enumerate(sorted_arrays):
        ...
        if first_min is not None:
            iters.insert((first_min, idx), it) #here using (first_min, idx) has tuple as key, second item 'it' is the iterator
