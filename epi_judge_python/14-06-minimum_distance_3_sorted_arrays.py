...
Logic:
    The trickt is to keep looking for smallest values in all the arrays and make combinations out of them. Smallest values will
    give you the smallest interval length.
    We fill up the the interval array with smallest picks from given arrays, finding the difference, then rreplacing the smallest among the element with the second smallest from same array it blonged from. Repeat process untill all arrays are exhausted.
...
def find_closest_elements_in_sorted_arrays_original(sorted_arrays: List[List[int]]
    ...
    for idx, sorted_array in enumerate(sorted_arrays):
        ...
        if first_min is not None:
            iters.insert((first_min, idx), it) #here using (first_min, idx) has tuple as key, second item 'it' is the value
            #cant use first_min alone as the key, since two array may have same values, hence we use idx too.
            # if  

    min_distance_so_far = float('inf')
    while True:
        min_value, min_idx = iters.min_key() #fetches min value key
        max_value = iters.max_key()[0]
        min_distance_so_far = min(max_value - min_value, min_distance_so_far)
        it = iters.pop_min()[1]#[1] returns the iiterator of min which was ejected
...
# below code proves that you can simply just create key without tuple to address duplicates. code returns ioncorrect answer for non tuple keys
def find_closest_elements_in_sorted_arrays(sorted_arrays: List[List[int]]
    ...
    for idx, sorted_array in enumerate(sorted_arrays_iter):
        ...
        if first_min is not None:
            iters.insert((first_min, idx), idx) #here using (first_min, idx) has tuple as key, second item 'it' is the value
            #cant use first_min alone as the key, since two array may have same values, hence we use idx too. 
    ...
    while True:
        ...
        max_value = iters.max_key()[0]
        min_distance_so_far = min(max_value - min_value, min_distance_so_far)
        it = iters.pop_min()[1]#[1] returns the index of min item which was ejected
        next_min = next(sorted_arrays_iter[it], None)
        # Return if some array has no remaining elements.
        if next_min is None:
            return typing.cast(int, min_distance_so_far)
        iters.insert((next_min, it), it)
