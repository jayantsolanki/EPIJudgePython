...
Logic:
    Median elements = X[(n+1)/2], if n is odd, else, (x[n/2] + x[n/2 + 1])/2, X is already sorted. Median is all about position
    ...
    be lessened.
    Most importantly, we only need a consistent way to access the median elements. Keeping the entire input 
...
def online_median_v2(sequence: Iterator[int]) -> List[float]:
    ...
    for x in sequence: # implementation on leetcode is opposite of this but is correct
        ...
        #max heap is supposed to get the smaller numbers from min_heap. It cannot get directly from the array sequence
