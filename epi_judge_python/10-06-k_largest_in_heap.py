...
Second hint: parent is always equal to greater than all its children, so start constructing the result array from parent at index 0
...
def k_largest_in_binary_heap_v2(A: List[int], k: int) -> List[int]:
    ...
    # a max heap.
    candidate_max_heap = []# heap is needed because children of the children 
    #in one subtree may or may not be larger than
    #an the children of the children in other subtree. Hence we keep 
    # pushing those children found into the max heap 
