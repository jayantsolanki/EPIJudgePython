...
#method 1 Divide and Conquer, causing TLE in last two, since the array is sorted those last two 
"""
...
def calculate_largest_rectangle_div_and_con(heights: List[int]) -> int:
    ...
    def calc_area(left, right):
        ...
        if left > right:
            ...
            #this is problematic, since it will change the index because it is using slice
...
Logic:
    ...
    We iterate from first index to last. Suppose we are at A[i], any rectangles which have greater height than A[i], 
    cannot be supported by A[i]'s height.
    That is A[i] block any further addition of pillar to a previous high rectangle. So, before appending index i,
    into stack, we start popping up the indices
    whose heights >= A[i], since we are now constructing piller for a different height rectangle, whose height == A[i]'s 
    height.
    Generalizing, we advance through indices, and add those without popping any previous items from stack, provided that 
    they can be supporting 
    pillars of the existing rectangle. And pop those items whose height >= current index to be added. Whenever a 
    building is removed from the active pillar set, we know  exactly how far to the right the largest rectangle 
    that is supports goes to. When we remove a blocked building from the active pillar set, to find how far 
    to the left its largest supported rectangle extends, we simple look for the closest active pillar that 
    has a lower height.
    Note, while, popping those bigger buildings, keep track of the width and height and calculate the area 
    of buildings being popped out.
...
def calculate_largest_rectangle(heights: List[int]) -> int:
    ...
    for i, h in enumerate(heights + [0]):#important, last value is zero, this will force stack to be emptied
        while pillar_indices and heights[pillar_indices[-1]] >= h:
            ...
            # - 1 because we are counting from the index ahead of the one which has been popped
...
Logic:
    For square we only consider width which is >= height, and then for calculating area, we just use height ** 2
"""
def calculate_largest_square(heights: List[int]) -> int:
    ...
    # rectangle area here.
    for i, h in enumerate(heights + [0]):#important, last value is zero, this will force stack to be emptied
        #pop those pillars which are now blocked by h
        while pillar_indices and heights[pillar_indices[-1]] >= h:
            height = heights[pillar_indices.pop()]
            # -1 because we are counting from the index ahead of the one which has been popped
            width = i if not pillar_indices else i - pillar_indices[-1] - 1 
