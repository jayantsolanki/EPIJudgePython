...
def longest_subarray_with_distinct_entries_original(A: List[int]) -> int:
    ...
    longest_dup_free_subarray_start_idx = result = 0#first variable stores the index of longest non duplicate subarray yet found
    for i, a in enumerate(A):
        # Defer updating dup_idx until we see a duplicate.
        if a in most_recent_occurrence:
            dup_idx = most_recent_occurrence[a]
            # A[i] appeared before. Did it appear in the longest current, that is , is it part of current subarray or not, if not then add it
            # subarray? this line is important, make sure that if longest_dup_free_subarray_start_idx is greater than dup_idx, then ignore it
            if dup_idx >= longest_dup_free_subarray_start_idx:#once the duplicate character's index found calcualte the updated length of subarray so far
...
def longest_subarray_with_distinct_entries(A: List[int]) -> int:
    ...
    for index, entry in enumerate(A):
        if entry in entries_pos:
            ...
            #in layman term, see if the dup-idx is on or after the subarraay start index, if yes, then update it, if before start of subarray then
            #we ignore it, since that number can be added into the subarray again, since it was part of other subarray
            if dup_idx >= longest_subarray_yet_start_index: #example [1, 2, 1, 3, 1, 2, 1], 2 is encountered at index 5, where start of subarray is at index 3
